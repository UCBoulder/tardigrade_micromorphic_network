...
Bug Fixes
=========
- Corrected bug in `micromorphic_network.cpp` in the map of `ddsdde` to `DDSDDE` due to using `spatialDimensions` instead
...
- Add Abaqus interface unit tests for checking the ``STATEV`` and ``PROPS`` vector lengths (:jira:`575`, :pull:`31`). By
  `Kyle Brindley`_.
- Add unit tests for error codes in ``micromorphic_network::sayHello`` (:jira:`334`, :pull:`32`). By `Kyle Brindley`_.
