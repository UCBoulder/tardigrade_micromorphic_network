...
.. code:: bash
   ...
   $ find path/to/conda/environment -name "libmicromorphic_network.so"

For instance, with the W-13 "release" environment on ``sstelmo``

.. code:: bash

   $ find /projects/aea_compute/release -name "libmicromorphic_network.so"
   /projects/aea_compute/release/lib64/libmicromorphic_network.so
...
``micromorphic_network/build/_deps/{error,vector,constitutive}_tools-build/``.

.. code:: bash

   $ pwd
   $ cp /path/to/micromorphic_network/build/src/cpp/{libmicromorphic_network.so} .
...
micromorphic_network requires 2 material constants and 2 state variables. The c++ micromorphic_network interface, material constants, and state
variables are described in the :ref:`sphinx_api`. The fixed expectations for the mooose interface are defined in the
"Variables" section of the :ref:`sphinx_api` for :ref:`micromorphic_network_source`. A complete discussion about the constants and their
meaning is not included here. Instead users are directed to calibrated material parameters found in micromorphic_network entries in the
...
The micromorphic_network project contains moose integration tests for the micromorphic_network moose interface. These tests perform actual simulations using the same dummy parameters used for unit and integration testing of the micromorphic_network c++ code. The micromorphic_network
moose input files used for integration testing can be found in the micromorphic_network source code repository with the following bash
command
...
.. warning::
   ...
   For calibrated material parameters, see the micromorphic_network entry for materials found in the `Granta/MIMS`_ `Material
